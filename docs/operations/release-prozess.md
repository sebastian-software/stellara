...
2. Ein Push auf `main` oder ein manueller `workflow_dispatch`-Aufruf startet `.github/workflows/release-please.yml`. Der Job läuft nur bei `RELEASE_AUTOMATION_ENABLED=true` und aktualisiert dann einen Release-PR.
3. Das Mergen des Release-PRs erzeugt einen Tag `vX.Y.Z` und veröffentlicht das zugehörige GitHub Release.
4. Das `release.published`-Event startet `.github/workflows/release.yml` bei stabilen Releases und Prereleases.
5. Der Workflow prüft Tag und `package.json`, baut das Image und veröffentlicht drei Tags:
   - exakte Version, zum Beispiel `0.1.15`;
   - Präfix der Version vor dem letzten Punkt, bei einer stabilen Version zum Beispiel `0.1`;
   - `latest`.
...
Normale Pushes nach `main`, Pull Requests und manuell erzeugte Tags veröffentlichen kein Container-Image. Bei einem veröffentlichten Prerelease setzt der Image-Workflow alle drei Tags ebenfalls; der abgeleitete Präfix ist dann nicht zwingend eine Minor-Serie.
...
Der Token wird benötigt, weil ein mit dem Standard-`GITHUB_TOKEN` erzeugter Tag keinen nachfolgenden Release-Workflow startet. Das Environment ist auf `main` beschränkt und benötigt keine manuelle Deployment-Freigabe. Bis das Environment-Secret gesetzt ist, bleibt die Repository-Variable `RELEASE_AUTOMATION_ENABLED` auf `false`; auch ein manueller `workflow_dispatch`-Aufruf überspringt dann den Job und erzeugt keinen Release-PR.

## Hinweise

- Versionstags werden nicht gelöscht oder neu erzeugt.
- Jeder veröffentlichte Release, auch ein Prerelease, aktualisiert `latest`. Der Tag ist daher weder stabilitätsgebunden noch für reproduzierbare Deployments geeignet.
